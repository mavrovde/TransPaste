...
5. **PR**: push the branch, `gh pr create`, then watch CI to green:
   `gh run watch <id> --exit-status`. Stages: Build → Test → Package, plus
   CodeQL (manual build mode — never re-enable default setup).
