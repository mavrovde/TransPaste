...
- `InputMonitor` — hotkey via **Carbon** `RegisterEventHotKey` (not NSEvent monitors); `start()` is idempotent (guards re-registration — "Setup Assistant…" in the menu calls it again). Capture: Accessibility API (`kAXSelectedTextAttribute`) first, then synthetic Cmd+A/Cmd+C with clipboard polling. `isMacroRunning` guards re-entrancy — every exit path must reset it.
